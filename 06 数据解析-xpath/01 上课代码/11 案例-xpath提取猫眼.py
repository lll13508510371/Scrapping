...
# 3.2 解析数据-->二次提取
divs = selector.xpath('//*[@class="board-card clearfix"]')  # 提取到所有的div标签
print(divs)
print(len(divs))

for div in divs:
    title = div.xpath('.//*[@class="title"]/text()').get()
    actors = div.xpath('.//*[@class="actors"]/text()').get()
    date = div.xpath('.//*[@class="date"]/text()').get()
    number = div.xpath('.//*[@class="number"]/text()').get()
